class MovieGenresController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @movie_genre.update(movie_genre_params)
        format.html { redirect_to @movie_genre, notice: "Movie genre was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to movie_genres_path, notice: "Movie genre was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_movie_genre
      @movie_genre = MovieGenre.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def movie_genre_params
      params.expect(movie_genre: [ :movie_id, :genre_id ])
    end
end
